...
export function mountStageEditor(getGame:()=>GameState,toast:(s:string,error?:boolean)=>void){
  ...
  function refresh(){
    ...
    q('[data-stats]').innerHTML=[['Baupreis',`${stats.cost} €`],['Unterhalt',`${stats.upkeep} €/h`],['Strom',`${stats.power} kW`],['Party',String(stats.party)],['Umgebung',String(stats.beauty)],['Elemente',`${design.parts.length}/96`]].map(([label,value])=>`<div><dt>${label}</dt><dd>${value}</dd></div>`).join('')
