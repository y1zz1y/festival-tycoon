...
import { COMPONENTS, AUDIENCE_KINDS, stageApronDepth, brandsFor, brandOf, defaultBrand, isTruss, GROUND_ONLY_KINDS, STACKABLE_KINDS, PHASE_NAMES, DECO_PATTERNS, STAGE_TILE_DETAIL, DECO_PATTERN_NAMES, STAGE_TILE_HEIGHT, defaultStageDesign, stageDesignIssue, stageDetailSize, stageStats, removeStagePart, migrateStageDesign, type StageDesign, type StagePart, type ComponentKind, type DecoPattern } from './game/stageDesign'
...
export function mountStageEditor(getGame:()=>GameState,toast:(s:string,error?:boolean)=>void){
  ...
  /** A make's name is shown as written, and one of them is literally `<no-name>`. */
  const plain=(text:string)=>text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')
  /**
   * The line under a part's name: what the cheapest one costs and that there is a choice,
   * or — for a part sold in one version — simply its price and what it is worth.
   */
  ...
  function updateHover(p:{x:number;y:number;alt:boolean}){
    ...
    candidate=kind?stagePlacement(design,{kind,brand:quality[kind]??defaultBrand(kind),rotation,color:q<HTMLInputElement>('[data-color]').value},local,auto&&hitId&&hitStep?{id:hitId,step:hitStep}:undefined):null
   ...
   * vanish entirely whenever the phase being edited has its Feuerwerk/Funken fader down — which
   * the default Warm-up does — so a fountain someone just placed looks broken rather than idle.
   * Previewing it at a low level keeps it on screen without pretending the fader is up.
   */
  ...
  panel.addEventListener('click',e=>{
    ...
    if(b.dataset.part){
      ...
      // Parts that come in one make have no menu to open — picking them just picks them.
